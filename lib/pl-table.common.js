...
module.exports = require("pl-table/lib/table-column");
    ...
    if (!fasle) {
      ...
      if (!this.store.states.newisAllSelected) {
        ...
        this.$parent.$nextTick(function () {
          _this.store.states.selection = _this.store.states.data; // 赋值全部数据
        });
...
// EXTERNAL MODULE: external "pl-table/lib/table-column"
var table_column_ = __webpack_require__(19);
...
/* harmony default export */ var plTablevue_type_script_lang_js_ = ({
    name: 'PlTable',
    components: { draggable: external_vuedraggable_default.a, ElTable: packages_table, ElPagination: packages_pagination, plTableColumn: table_column_default.a },
    props: {
    ...
    methods: {
            ...
            if (this.recordTableSelect) {
                if (selection.length === 0) {
                    ...
                    this.datas.forEach(function (item) {
                        _this2.tableSelectData.forEach(function (is, index) {
                            if (item.id === is) {
                                _this2.cancelSelectData.push(item.id);
                ...
                if (selection.length > 0) {
                    selection.forEach(function (item) {
                        _this2.cancelSelectData.forEach(function (is, index) {
                            if (item.id === is) {
                                _this2.cancelSelectData.splice(index, 1);
                            }
                        });
                    });
                }
            ...
            if (row.id && this.recordTableSelect) {
                ...
                if (falsData.length === 0) {
                    this.tableSelectData.forEach(function (item, index) {
                        if (item === row.id) {
                            _this3.cancelSelectData.push(row.id);
                ...
                // 当前节点被开启
                if (falsData.length > 0) {
                    this.cancelSelectData.forEach(function (item, index) {
                        if (item === row.id) {
                            _this3.cancelSelectData.splice(index, 1);
                        }
                    });
                }
                // 这就是整个表格当前选中项（包含分页）
        ...
        handleSelectionChange: function handleSelectionChange(val) {
            ...
            if (this.recordTableSelect) {
                ...
                val.forEach(function (value) {
                    _this4.tableSelectData.push(value.id);
                });
                // 数组去重然后再赋值
        ...
        toggleRowSelection: function toggleRowSelection(rows) {
            if (rows && rows.constructor === Array) {
                ...
                if (this.recordTableSelect) {
                    // 筛选过滤数组
                    var filterFun = function filterFun(data, isd) {
                        return data.filter(function (item) {
                            return item.row.id !== isd;
                        });
                    };

                    // 去重过后的值
                    var datasFiler = this.removalDataObj(rows, 'id');
                    this.cancelSelectData.forEach(function (isd) {
        ...
        // 用于多选表格，切换某一行的选中状态
        newtoggleRowSelections: function newtoggleRowSelections() {
            var _this5 = this;

            if (this.rowSelectionsData.length > 0 && this.$refs.singleTable) {
                this.rowSelectionsData.forEach(function (item) {
                    if (item.selected) {
                        _this5.$refs.singleTable.toggleRowSelection(item.row, item.selected);
                    } else if (item.selected === false) {
                        _this5.$refs.singleTable.toggleRowSelection(item.row, false);
                    } else if (item.selected === undefined) {
                        _this5.$refs.singleTable.toggleRowSelection(item.row);
                    }
                });
            }
        },
        ...
        getSummaries: function getSummaries(param) {
            ...
            if (typeof this.summaryMethod === 'function' && this.summaryMethod) {
                ...
                columns.forEach(function (column, index) {
                    ...
                    // 需要合并的选项
                    var flas = _this6.totalOption.filter(function (item) {
        ...
        setHeight: function setHeight() {
            ...
            if (this.$refs.tableBox) {
                ...
                this.$nextTick(function () {
                    _this8.maxHeight = tableBox - myPagination;
                    // 随数据高度变化
                    if (_this8.heightChange) {
                        _this8.$nextTick(function () {
                            if (!_this8.$refs.singleTable) return;
                            _this8.$refs.singleTable.$el.childNodes.forEach(function (item) {
                                // 表体高
                                    ...
                                    if (parseInt(item.style.height)) {
                                        if (item.children[0]) {
                                            ...
                                            if (tablebodyH > 0) {
                                                ...
                                                // })
                                                // 如果出现滚动条(就要把表体高度加上9,   9就是滚动条样式的高度，) 9
                                                _this8.maxHeight = tableBox - myPagination - tablebodyH + 9;
                                                // 如果没数据 就加上提示的高度
                                                if (_this8.datas.length === 0) {
                                                    _this8.maxHeight = _this8.maxHeight + 82;
                                                }
                                            }
                                        }
                                    }
                    ...
                    _this8.$emit('load-complete', _this8.$refs.singleTable);
                });
            }
        },
                ...
                event.path.forEach(function (item) {
                    if (item && item.classList) {
                        item.classList.forEach(function (its) {
                            if (its === 'current-row') {
                                _this9.$refs.singleTable.setCurrentRow();
                            }
                        });
                    }
                });
        ...
        datas: {
            ...
            handler: function handler(val) {
                ...
                this.$nextTick(function () {
                    _this10.setHeight();
                    // 去设置选中项
                    _this10.newtoggleRowSelections();
                });
            }
        },
        dialogData: {
...
var components = [packages_table_column, pl_table];
